...
function moveCarousel(direction, carouselNumber) {
    ...
    const carouselContainer = carousels[carouselNumber - 1].querySelector('.carrossel-container');
    const items = carousels[carouselNumber - 1].querySelectorAll('.caixa-video');
    const itemWidth = items[0].offsetWidth + 20; // Largura do item mais margem
    const totalItems = items.length;
    const visibleItems = Math.floor(carousels[carouselNumber - 1].clientWidth / itemWidth);

    currentIndexes[carouselNumber - 1] += direction;

    if (currentIndexes[carouselNumber - 1] < 0) {
        currentIndexes[carouselNumber - 1] = totalItems - visibleItems;
    } else if (currentIndexes[carouselNumber - 1] >= totalItems) {
        currentIndexes[carouselNumber - 1] = 0;
    }

    const offset = -currentIndexes[carouselNumber - 1] * itemWidth;

...
<header class="header" data-header>
    ...
    <div class="titulo-laranja">
        <h3>Explore Nossa Galeria de Vídeos Educativos</h3>
    </div>

 <!-- Carrossel de Vídeos -->
<div class="carrossel">
    <button class="prev" onclick="moveCarousel(-1, 1)">&#10094;</button>
    <div class="carrossel-container">
        <?php
        require_once 'db_connect.php';
        $sql = "SELECT * FROM videos_multimidia ORDER BY data_adicao DESC";
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<div class="caixa-video">';
                $imagemSrc = !empty($row['caminho_imagem']) ? htmlspecialchars($row['caminho_imagem']) : './img/vid.png';
                echo '<img src="../../Dashboard Do Admin/uploads/' . $imagemSrc . '" alt="Ícone de Vídeo">';
                echo '<h4>' . htmlspecialchars($row['titulo']) . '</h4>';
                echo '<div class="linha-laranja"></div>';
                echo '<p class="autor">Autor do Vídeo</p>';
                echo '<p>' . htmlspecialchars($row['descricao']) . '</p>';
                echo '<a href="' . htmlspecialchars($row['link']) . '" target="_blank"><button class="download-btn">Assistir</button></a>';
                echo '</div>';
            }
        } else {
            echo "<p style='color: red; text-align: center;'>Nenhum vídeo encontrado.</p>";
        }
        ?>
    </div>
    <button class="next" onclick="moveCarousel(1, 1)">&#10095;</button>
</div>

...
<header class="header" data-header>
    ...
    <div class="mini-banner">
        <img src="./img/banner5.png" alt="Banner Image">
    </div>

    <!-- Título "Dicas" -->
    <div class="etapas">
        <h2>Busque Referências para seu TCC</h2>
    </div>

    <!-- Barra de Pesquisa -->
<div class="search-bar">
    <input type="text" placeholder="Pesquisar dicas..." id="searchInput">
    <button type="button" onclick="searchTips()">Pesquisar</button>
</div>
...
<div class="file-box">
    ...
    $sql = "SELECT id, titulo, introducao, arquivo FROM referencias";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            if ($count % 3 == 0 && $count != 0) {
                echo '</div><div class="file-row">';
            }
            echo '<div class="file-item" data-titulo="' . htmlspecialchars($row["titulo"]) . '">';
            echo '<h3>' . htmlspecialchars($row["titulo"]) . '</h3>';
            echo '<p>' . htmlspecialchars($row["introducao"]) . '</p>';
            // Link para abrir o PDF em uma nova aba
            echo '<a href="../../Dashboard Das Empresas/' . htmlspecialchars($row["arquivo"]) . '" target="_blank" class="download-btn" id="pdf-link">Abrir PDF</a>';
            echo '</div>';
            $count++;
        }
        echo '</div>';
...
// Função de pesquisa
function searchTips() {
    var input = document.getElementById("searchInput"); // Campo de pesquisa
    var filter = input.value.toUpperCase(); // Texto da pesquisa (em maiúsculas para comparação)
    var fileItems = document.querySelectorAll(".file-item"); // Seleciona todos os itens de arquivo

    var found = false; // Variável para verificar se algum item foi encontrado

    // Percorre todos os itens da caixa de arquivos
    fileItems.forEach(function(item) {
        var title = item.getAttribute("data-titulo").toUpperCase(); // Título do item
        
        // Verifica se o título contém o texto da pesquisa
        if (title.indexOf(filter) > -1) {
            item.style.display = ""; // Exibe o item se encontrado
            found = true;
        } else {
            item.style.display = "none"; // Esconde o item se não encontrado
        }
    });

    // Desativa o botão de "Abrir PDF" se nenhum item foi encontrado
    var pdfLinks = document.querySelectorAll("#pdf-link");
    pdfLinks.forEach(function(link) {
        if (!found) {
            link.style.pointerEvents = "none"; // Desabilita o clique
            link.style.opacity = 0.5; // Deixa o link visualmente desabilitado
        } else {
            link.style.pointerEvents = "auto"; // Habilita o clique
            link.style.opacity = 1; // Restaura a opacidade
        }
    });
}
</script>
